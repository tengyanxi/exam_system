...
from DataOperation import judgeIn, writeData
...
class register(Ui_registerWindow,QMainWindow):
    ...
    def showTip2(self):   #注册界面用户名已被注册提示
        if judgeIn(self.lineEdit_register_name.text()):
    ...
    def confirmButton_click(self):
        if not judgeIn(self.lineEdit_register_name.text()) and self.lineEdit_register_pwd1.text() == self.lineEdit_register_pwd2.text():
            writeData(self.lineEdit_register_name.text(), self.lineEdit_register_pwd1.text())
